...
export function StarButton({
  ...
  const sharedClass = cn(
    "relative z-[3] overflow-hidden inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full text-sm font-bold transition-opacity cursor-pointer group/star-button",
    "px-8 py-4",
    className
  );
  ...
  const inner = (
    <>
      ...
      <div
        className="absolute inset-0 z-[4] overflow-hidden rounded-[inherit] opacity-100"
        style={{ borderWidth: "var(--border-width)", borderColor: "rgba(200,169,110,0.3)", borderStyle: "solid" }}
        aria-hidden="true"
      >
        <StarBackground />
      </div>
      {/* Label */}
      <span className="z-10 relative text-[#0D0F10] font-bold" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: "15px" }}>
        {children}
      </span>
    </>
  );
  ...
  if (href) {
    return (
      <a
        ...
        target={target}
        rel={rel}
        style={{ ...sharedStyle, background: "linear-gradient(90deg, #c8a96e 0%, #e2c98a 100%)", textDecoration: "none" }}
  ...
  return (
    <button
      ...
      style={{ ...sharedStyle, background: "linear-gradient(90deg, #c8a96e 0%, #e2c98a 100%)", border: "none" }}
      className={sharedClass}
      onClick={onClick}
    >
      {inner}
